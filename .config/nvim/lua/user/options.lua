...
-- Auto trim trailing whitespace & blank-only lines before save
...
-- Format staged files command
vim.api.nvim_create_user_command("FormatStaged", function()
	-- Get staged files
	...
	for file in result:gmatch("[^\r\n]+") do
		if vim.fn.filereadable(file) == 1 then
			-- open without switching window
			vim.cmd("silent edit " .. vim.fn.fnameescape(file))

			-- write → triggers your format-on-save
			vim.cmd("silent write")

			-- re-stage file (because saving un-stages changes)
			os.execute("git add " .. vim.fn.shellescape(file))
